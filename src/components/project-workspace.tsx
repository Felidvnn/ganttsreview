...
import { filterProjectTasks, taskDurationDays, type TaskDeadlineFilter, type TaskStatusFilter } from "@/lib/task-filters";
...
import { ProjectBusinessCase } from "./project-business-case";
...
type ProjectView = "gantt" | "list" | "board" | "milestones" | "followups" | "notes" | "delays" | "business" | "reports" | "activity";
...
function hierarchyLevelLabel(depth: number) { return depth === 0 ? "Tarea principal" : `Nivel ${depth}`; }
...
function polishedReportHtml(project: Project, tasks: Task[], statuses: ProjectTaskStatus[]) {
  ...
  const rows = sections.map((section) => {
    ...
    return `<tr class="section"><td colspan="9"><strong>${htmlCell(section)}</strong><span>${sectionTasks.length} actividades</span></td></tr>${sectionTasks.map((task) => {
      const depth = taskDepth(task, ordered);
      const priority = task.priority === 3 ? "Alta" : task.priority === 1 ? "Baja" : "Media";
      return `<tr class="task level-${depth}"><td><div class="tree depth-${depth}"><i></i><span><strong>${htmlCell(task.title)}</strong><small>${hierarchyLevelLabel(depth)}</small></span></div></td><td>${htmlCell(task.owners?.map((owner) => owner.name).join(", ") || task.owner.name)}</td><td><span class="priority p-${priority.toLowerCase()}">${priority}</span></td><td><span class="state" style="--state:${statuses.find((item) => item.status === task.status)?.color || "#68766f"}">${htmlCell(statusLabel(task.status, statuses))}</span></td><td><div class="bar"><i style="width:${task.progress}%;background:${task.color || "#2f7669"}"></i></div><b>${task.progress}%</b></td><td><b>${htmlCell(task.startDate || "Sin inicio")}</b></td><td><b>${htmlCell(task.dueDate || "Sin término")}</b></td><td><b>${taskDurationDays(task)} d</b></td><td>${htmlCell(`${task.description || "—"}${task.actualCompletionDate ? ` · Fecha real: ${task.actualCompletionDate}` : ""}`)}</td></tr>`;
    }).join("")}`;
  }).join("");
  const roots = ordered.filter((task) => !task.parentId || !ordered.some((parent) => parent.id === task.parentId));
...
function excelWorkbook(project: Project, tasks: Task[], statuses: ProjectTaskStatus[]) {
  ...
  const header = ["Nivel", "Jerarquía", "Tarea", "Tarea padre", "Sección", "Responsables", "Prioridad", "Estado", "Avance (%)", "Inicio", "Fin", "Duración (días)", "Fecha real", "Tipo", "Descripción"].map((value) => cell(value, "Header")).join("");
  const rows = ordered.map((task) => {
    ...
    const values: Array<[unknown, string?]> = [
      [hierarchyLevelLabel(depth)], [taskHierarchyPath(task, ordered)], [task.title, `Level${depth}`], [parent?.title || ""], [taskDisplaySection(task, ordered)], [task.owners?.map((owner) => owner.name).join(", ") || task.owner.name], [priority, `Priority${priority}`], [statusLabel(task.status, statuses)], [task.progress, "Number"], [task.startDate || "Sin inicio", "Date"], [task.dueDate || "Sin término", "Date"], [taskDurationDays(task), "Number"], [task.actualCompletionDate || "", "Date"], [task.taskTypeName || (task.isMilestone ? "Hito" : "Tarea")], [task.description || ""],
    ];
    return `<Row>${values.map(([value, style]) => cell(value, style)).join("")}</Row>`;
  }).join("");
  return `<?xml version="1.0"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Styles><Style ss:ID="Default" ss:Name="Normal"><Alignment ss:Vertical="Center"/><Font ss:FontName="Aptos" ss:Size="10"/></Style><Style ss:ID="Text"><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#E5EAE7"/></Borders></Style><Style ss:ID="Header"><Font ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#245F55" ss:Pattern="Solid"/><Alignment ss:WrapText="1"/></Style><Style ss:ID="Level0"><Font ss:Bold="1"/></Style><Style ss:ID="Level1"><Alignment ss:Indent="2"/><Interior ss:Color="#F8FBF9" ss:Pattern="Solid"/></Style><Style ss:ID="Level2"><Alignment ss:Indent="4"/><Interior ss:Color="#F2F7F4" ss:Pattern="Solid"/></Style><Style ss:ID="Level3"><Alignment ss:Indent="6"/><Interior ss:Color="#EDF4F0" ss:Pattern="Solid"/></Style><Style ss:ID="PriorityAlta"><Font ss:Color="#A34F42" ss:Bold="1"/><Interior ss:Color="#FFF0ED" ss:Pattern="Solid"/></Style><Style ss:ID="PriorityMedia"><Font ss:Color="#97662D"/><Interior ss:Color="#FFF5E7" ss:Pattern="Solid"/></Style><Style ss:ID="PriorityBaja"><Font ss:Color="#617B72"/><Interior ss:Color="#EDF3F1" ss:Pattern="Solid"/></Style><Style ss:ID="Number"><NumberFormat ss:Format="0"/><Alignment ss:Horizontal="Center"/></Style><Style ss:ID="Date"><Alignment ss:Horizontal="Center"/></Style></Styles><Worksheet ss:Name="Planificación"><Table><Column ss:Width="92"/><Column ss:Width="210"/><Column ss:Width="220"/><Column ss:Width="180"/><Column ss:Width="120"/><Column ss:Width="130"/><Column ss:Width="75"/><Column ss:Width="90"/><Column ss:Width="70"/><Column ss:Width="82"/><Column ss:Width="82"/><Column ss:Width="70"/><Column ss:Width="82"/><Column ss:Width="90"/><Column ss:Width="260"/><Row ss:Height="28">${header}</Row>${rows}</Table><WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel"><FreezePanes/><FrozenNoSplit/><SplitHorizontal>1</SplitHorizontal><TopRowBottomPane>1</TopRowBottomPane><AutoFilter x:Range="R1C1:R${ordered.length + 1}C15" xmlns:x="urn:schemas-microsoft-com:office:excel"/></WorksheetOptions></Worksheet></Workbook>`;
}
...
export function ProjectWorkspace({ project, initialTasks, canEdit }: { project: Project; initialTasks: Task[]; canEdit: boolean }) {
  ...
  const filteredTasks = useMemo(
    ...
    [deadlineFilter, statusFilter, tasks],
  );
  const filtersActive = statusFilter !== "all" || deadlineFilter !== "all";
  ...
  const exportExcel = () => downloadBlob(`${project.code}-planificacion.xls`, excelWorkbook(project, filteredTasks, projectStatuses), "application/vnd.ms-excel;charset=utf-8");
  const exportHtml = () => downloadBlob(`${project.code}-informe.html`, polishedReportHtml(project, filteredTasks, projectStatuses), "text/html;charset=utf-8");
  const exportPdf = () => {
    const popup = window.open("", "_blank", "width=1100,height=800");
    if (!popup) return;
    popup.document.open(); popup.document.write(polishedReportHtml(project, filteredTasks, projectStatuses)); popup.document.close();
    window.setTimeout(() => { popup.focus(); popup.print(); }, 350);
  };
  const exportPng = () => {
    const orderedTasks = sortTasksByDate(filteredTasks);
    ...
    const metrics = [{ label: "AVANCE", value: `${currentProgress}%` }, { label: "TAREAS VISIBLES", value: String(filteredTasks.length) }, { label: "PENDIENTES", value: String(filteredTasks.filter((task) => task.status !== "done").length) }, { label: "BLOQUEADAS", value: String(filteredTasks.filter((task) => task.status === "blocked").length) }];
    ...
    visualRows.forEach((row, index) => {
      ...
      context.fillStyle = "#263a31"; context.font = `${depth ? "500" : "600"} 13px Segoe UI, Arial`; context.fillText(task.title.slice(0, 48), taskTextX, y - 3); context.fillStyle = "#83908a"; context.font = "9px Segoe UI, Arial"; context.fillText(`${depth ? hierarchyLevelLabel(depth) : taskDisplaySection(task, orderedTasks)}  ·  ${task.owner.name}  ·  ${task.priority === 3 ? "Alta" : task.priority === 1 ? "Baja" : "Media"}  ·  Inicio ${task.startDate || "—"}  ·  Fin ${task.dueDate || "—"}`, taskTextX, y + 14);
  ...
  return <>
    <section className="project-summary-strip">
      <div><small>AVANCE INFORMADO</small><b>{currentProgress}%</b><span className="summary-progress"><i style={{ width: `${currentProgress}%`, background: project.color }} /></span></div>
    ...
    {view === "business" && <ProjectBusinessCase projectId={project.id} canEdit={canEdit} />}
