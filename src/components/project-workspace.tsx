...
export function ProjectWorkspace({ project, initialTasks, canEdit }: { project: Project; initialTasks: Task[]; canEdit: boolean }) {
  ...
  return <>
    ...
    {selectedTask && <TaskEditor key={selectedTask.id} task={tasks.find((task) => task.id === selectedTask.id) || selectedTask} allTasks={tasks} sections={sections.length ? sections : ["General"]} members={members} canEdit={canEdit} projectStatuses={projectStatuses} onClose={() => setSelectedTask(null)} onUpdated={updateLocalTask} onCreated={createLocalTask} onDeleted={deleteLocalTask} onSelectTask={setSelectedTask} />}
