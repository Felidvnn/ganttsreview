...
import { applyTaskRollups, sortTasksByDate, sortTasksManual, taskDepth, taskDisplaySection } from "@/lib/task-order";
...
export function ProjectWorkspace({ project, initialTasks, canEdit }: { project: Project; initialTasks: Task[]; canEdit: boolean }) {
  ...
  const exportExcel = async () => {
    ...
    try {
      const workbook = await buildProjectExcelWorkbook(project, filteredTasks, projectStatuses, sections, filtersActive, tasks);
      downloadBlob(`${project.name.trim().replace(/[\\/:*?"<>|]+/g, "-") || "proyecto"}-gantt.xlsx`, workbook as BlobPart, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    } catch (cause) {
      window.alert(cause instanceof Error ? cause.message : "No se pudo generar el libro Excel.");
    } finally {
  ...
  return <>
    ...
    {view === "reports" && <section className="report-grid"><button onClick={exportPdf}><span className="report-icon pdf"><FileText /></span><div><b>Informe PDF</b><small>Abre la versión imprimible para guardar como PDF.</small></div><Download /></button><button onClick={exportHtml}><span className="report-icon html"><FileCode2 /></span><div><b>Informe HTML</b><small>Documento autocontenido para compartir o archivar.</small></div><Download /></button><button onClick={exportPng}><span className="report-icon image"><FileImage /></span><div><b>Imagen PNG</b><small>Captura gráfica del cronograma y sus cápsulas.</small></div><Download /></button><button onClick={exportExcel} disabled={excelExporting}><span className="report-icon csv"><FileSpreadsheet /></span><div><b>{excelExporting ? "Preparando Excel…" : "Libro Excel profesional"}</b><small>Gantt visual diaria y resumen ejecutivo en hojas separadas.</small></div><Download /></button></section>}
