...
export function ProjectsView({ projects }: { projects: Project[] }) {
  ...
  return <>
    ...
    <div className="filter-bar"><div className="tab-filter">{["Todos", "Equipo", "Compartido", "Privado", "En riesgo"].map((item) => <button key={item} className={filter === item ? "active" : ""} onClick={() => setFilter(item)}>{item}{item === "En riesgo" && <span>{projects.filter((project) => project.health !== "healthy").length}</span>}</button>)}</div><div className="filter-actions"><label className="small-search"><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar proyecto" /></label></div></div>
