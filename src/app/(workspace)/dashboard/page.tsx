...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ scope?: string }> }) {
  ...
  const teamPeople = new Map<string, {
    ...
    focus: Array<{ id: string; title: string; projectId: string; project: string; dueDate: string | null; overdue?: boolean; owner: string }>;
  }>();
  if (scope === "team") {
    // Group membership is the only source of engineers. Task assignees may be
    // providers, contacts or collaborators and must not create team cards.
    for (const person of teamRoster.filter((member) => member.role === "engineer" && member.id !== shell?.id)) {
    ...
    for (const project of teamProjects) {
      const current = project.createdBy ? teamPeople.get(project.createdBy) : undefined;
      if (!current) continue;
      current.projects.add(project.id);
      current.progressTotal += project.progress;
      current.progressProjects += 1;
    }
    // Accountability follows the project creator, while each task keeps its
    // real assignee as context. Include all unfinished Gantt tasks, not only
    // tasks assigned to the engineer or tasks due during the current week.
    for (const item of week.items.filter((entry) => entry.source === "task" && !entry.done && Boolean(entry.projectId && teamProjectIds.has(entry.projectId)))) {
      const project = teamProjects.find((entry) => entry.id === item.projectId);
      const current = project?.createdBy ? teamPeople.get(project.createdBy) : undefined;
      if (!current) continue;
      current.pending += 1;
      if (item.overdue) current.overdue += 1;
      current.focus.push({ id: item.id, title: item.title, projectId: item.projectId!, project: item.project, dueDate: item.dueDate, overdue: item.overdue, owner: item.owner || "Sin responsable" });
    }
  }
  const teamSummary = [...teamPeople.values()].map((person) => ({
    ...person,
    focus: person.focus.sort((left, right) => Number(Boolean(right.overdue)) - Number(Boolean(left.overdue)) || (left.dueDate || "9999-12-31").localeCompare(right.dueDate || "9999-12-31")),
  })).sort((left, right) => right.overdue - left.overdue || right.pending - left.pending || left.name.localeCompare(right.name, "es"));
    ...
    <div className="dashboard-page">
      ...
      <section className="metric-grid">
        {scope === "team" ? <>
          ...
          <article className="metric-card warning"><span className="metric-icon amber"><AlertTriangle /></span><div><small>ATRASOS DEL EQUIPO</small><b>{teamSummary.reduce((total, person) => total + person.overdue, 0)}</b><p>{attentionCount} proyectos requieren atención</p></div></article>
      ...
      {scope === "team" && <section className="panel team-lead-panel">
        <div className="panel-head"><div><span className="eyebrow">VISTA DE JEFATURA</span><h3>Avance y tareas por ingeniero</h3></div><Link href="/team">Administrar equipo <ArrowRight size={15} /></Link></div>
        {teamSummary.length ? <div className="team-member-list">{teamSummary.map((person) => {
          ...
          return <article className="team-member-row" key={person.id}>
            <header className="team-member-head"><Avatar person={{ id: person.id, name: person.name, initials: person.initials, role: "Ingeniero", color: person.color }} /><span className="team-member-copy"><b>{person.name}</b><small>{person.projects.size} {person.projects.size === 1 ? "proyecto visible" : "proyectos visibles"} · {person.pending} tareas pendientes</small></span><span className={`team-member-alert ${person.overdue ? "late" : "clear"}`}>{person.overdue ? `${person.overdue} atrasada${person.overdue === 1 ? "" : "s"}` : "Al día"}</span></header>
            <div className="team-member-projects"><span className="team-card-label">PROYECTOS A CARGO</span>{personProjects.slice(0, 2).map((project) => { const projectPending = person.focus.filter((item) => item.project === project.name).length; return <Link href={`/projects/${project.id}`} key={project.id}><i style={{ background: project.color }} /><span><b>{project.name}</b><small>{projectPending ? `${projectPending} tarea${projectPending === 1 ? "" : "s"} pendiente${projectPending === 1 ? "" : "s"}` : project.health === "healthy" ? "Avance dentro de lo esperado" : project.health === "risk" ? "Requiere seguimiento" : "Proyecto con atraso"}</small></span><strong>{project.progress}%</strong></Link>; })}{!personProjects.length && <span className="team-member-no-projects"><FolderKanban size={13} /> Sin proyectos visibles para jefatura</span>}{personProjects.length > 2 && <small className="team-member-more">+{personProjects.length - 2} proyectos adicionales</small>}</div>
            <div className="team-member-focus team-member-focus-readable"><span className="team-card-label">TAREAS ATRASADAS Y FUTURAS</span>{person.focus.slice(0, 4).map((item) => { const dueLabel = item.dueDate ? format(new Date(`${item.dueDate}T12:00:00`), "dd MMM") : null; const scheduleLabel = item.overdue ? `Atrasada${dueLabel ? ` · ${dueLabel}` : ""}` : dueLabel ? `Vence ${dueLabel}` : "Sin fecha límite"; return <Link href={`/projects/${item.projectId}`} className={item.overdue ? "team-focus-row overdue" : "team-focus-row"} key={item.id}><i className="team-focus-check" /><span className="team-focus-copy"><b>{item.title}</b><small><em style={{ background: personProjects.find((project) => project.id === item.projectId)?.color || "#2f7669" }} />{item.project}</small></span><span className="team-focus-owner">{item.owner}</span><span className="team-focus-due"><Clock3 size={11} />{scheduleLabel}</span></Link>; })}{!person.focus.length && <span className="team-member-no-focus"><Check size={12} /><em>Sin tareas pendientes en sus proyectos</em></span>}{person.focus.length > 4 && <small className="team-member-more">+{person.focus.length - 4} tareas adicionales</small>}</div>
