...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ scope?: string }> }) {
  ...
  const teamPeople = new Map<string, {
    ...
    focus: Array<{ id: string; title: string; project: string; dueDate: string | null; overdue?: boolean; owner: string }>;
  }>();
  if (scope === "team") {
    ...
    for (const item of week.items.filter((entry) => entry.source === "task" && !entry.done && Boolean(entry.projectId && teamProjectIds.has(entry.projectId)))) {
      ...
      current.focus.push({ id: item.id, title: item.title, project: item.project, dueDate: item.dueDate, overdue: item.overdue, owner: item.owner || "Sin responsable" });
    }
  }
      ...
      {scope === "team" && <section className="panel team-lead-panel">
        ...
        {teamSummary.length ? <div className="team-member-list">{teamSummary.map((person) => {
          ...
          return <article className="team-member-row" key={person.id}>
            ...
            <div className="team-member-focus"><span className="team-card-label">TAREAS ATRASADAS Y FUTURAS</span>{person.focus.slice(0, 3).map((item) => { const dueLabel = item.dueDate ? format(new Date(`${item.dueDate}T12:00:00`), "dd MMM") : null; const scheduleLabel = item.overdue ? `Atrasada${dueLabel ? ` · ${dueLabel}` : ""}` : dueLabel ? `Vence ${dueLabel}` : "Sin fecha límite"; return <span key={item.id}><i className={item.overdue ? "late" : ""} /><span><b>{item.title}</b><small>{item.project} · Responsable: {item.owner} · {scheduleLabel}</small></span></span>; })}{!person.focus.length && <span className="team-member-no-focus"><Check size={12} /><em>Sin tareas pendientes en sus proyectos</em></span>}{person.focus.length > 3 && <small className="team-member-more">+{person.focus.length - 3} tareas adicionales</small>}</div>
